...
contract A3SQueue is IA3SQueue{
    ...
    function pushIn(address _addr) external{
        ...
        require(address_node[_addr].addr == address(0), "A3S: address played and invalid to queue");
        ...
        if(curQueueLength > maxQueueLength){
            ...
            uint16 _prevDayCount = _getPrevDayInQueueCount();
            console.log("Prev Day total minted: ", _prevDayCount);
            if(_prevDayCount >= 200){
                uint16 _extended =  _getExtendLength(_prevDayCount);
                console.log("Extended length: ", _prevDayCount);
    ...
    function jumpToSteal(address jumpingAddr, address stolenAddr) external {
        ...
        emit JumpToSteal(jumpingAddr, stolenAddr, _balance, headIdx, tailIdx, address_node[stolenAddr].stat);
    }
    ...
    function pushOut() public{
        ...
        address_node[headIdx].stat = queueStatus.PENDING;
        address_node[headIdx].outQueueTime = uint64(block.timestamp);
        address_node[headIdx].balance = uint256(_getTokenAmount(headIdx));
        //Update the headIdx
    ...
    function getCurrentPosition(address _addr) external view returns(uint256 ans){
        ans = 0;
    ...
    function claim(address _addr) external {
        //require(_addr == msg.sender, "A3S: can ONLY calim own address");
        require(address_node[_addr].outQueueTime > 0, "A3S: NOT valid to calim - not pushed");
        require(address_node[_addr].stat == queueStatus.PENDING, "A3S: ONLY pending status could be claimed");
        require(uint64(block.timestamp) - address_node[_addr].outQueueTime < 3 days, "A3S: NOT valid to calim - out of queue exceed 3 days");
        ...
        emit Claim(_addr, _balance, address_node[_addr].stat);
    }
    ...
    function _steal(address _addr) internal returns(uint256){
        ...
        require(address_node[_addr].stat == queueStatus.PENDING, "A3S: ONLY pending status could be stolen");
        require(uint64(block.timestamp) - address_node[_addr].outQueueTime >= 3 days, "A3S: NOT valid to steal - not reaching 3 days");
    ...
    function _getPrevDayInQueueCount() internal view returns(uint16 count){
        count = 0;
        address pointer = tailIdx;
        console.log("In Queue time: ", address_node[pointer].inQueueTime);
        console.log("Current block time: ", block.timestamp);
        console.log("Current block time - 1 days: ", block.timestamp - 1 days);
        while(address_node[pointer].inQueueTime >= block.timestamp - 1 days){
            //Unlock to claim
            pointer = address_node[pointer].next;
            count += 1;
        }
    ...
    function _getExtendLength(uint16 prevDayIncreCount) internal pure returns(uint16 extendLength){
        uint16 n =  prevDayIncreCount / 100;
        extendLength = 0;
        for(uint16 i = 1; i< n; i++){
            extendLength += (prevDayIncreCount * (10 - 100/(10+i)))/ 10;
        }
    }

    function _getTokenAmount(address _addr) internal view returns(uint256 amount){
    ...
    function log_2 (uint x) internal pure returns (uint y){
        assembly {
                let arg := x
                x := sub(x,1)
                x := or(x, div(x, 0x02))
